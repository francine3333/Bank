...
// Fetching savingstype and savingoptions options dynamically from the database for display 
$querySavings = "SELECT savingstype, id FROM accbalance WHERE username = ?";
$stmtSavings = $conn->prepare($querySavings);
$stmtSavings->bind_param("s", $username);
$stmtSavings->execute();
$stmtSavings->bind_result($savingstype, $id);

$savingsOptions = '';
while ($stmtSavings->fetch()) {
    $savingsOptions .= '<option value="' . htmlspecialchars($id) . '">' . htmlspecialchars($savingstype) . ' (ID: ' . htmlspecialchars($id) . ')</option>';
}
$stmtSavings->close();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Update balance in the database for savingoptions based on selected id
...
<head>
    ...
    <title>Cash-in - Wealth Finance Management</title>
            ...
            <div class="row justify-content-center">
                <div class="col-md-6">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
                        ...
                        <div class="form-group">
                            <label for="savings">Select Savings Type and ID:</label>
                            <select id="savings" name="id" required>
                                <?php echo $savingsOptions; ?>
                            </select>
                        </div>
                        <div class="form-group">
